# backend.py
"""
All CSV backend logic: normalisation, column detection, transforms, saving.
"""
import os
import re
import csv
...
def transform_beyonic(df: pd.DataFrame, sequence_choice: str, log=None) -> pd.DataFrame:
    if log:
        log("**Applying Beyonic transformation...**")
    required_cols = {
...
def transform_flexipay(df: pd.DataFrame, sequence_choice: str, log=None) -> pd.DataFrame:
    if log:
        log("**Applying FlexiPay transformation & filtering...**")
    candidates = {
    ...
    if "Transaction Type" in new_df.columns and "Status" in new_df.columns:
        ...
        log("**FlexiPay Warning**: Missing Type/Status columns, skipping filter.")
...
def save_chunks(df_out: pd.DataFrame, base_output_dir: str, mode: str = "Beyonic", log=None) -> list:
    ...
    for part in range(num_parts):
        ...
        path   = os.path.join(base_output_dir, fname)
        try:
            chunk.to_csv(
            ...
            if log:
                log(f"**Saved** {len(chunk)} rows → {fname}")
        except Exception as e:
            if log:
