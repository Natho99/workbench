...
# main.py
"""
4G WORKBENCH — Entry Point (macOS Optimized)
"""
import os
import subprocess
import sys
...
# Import backend/config - Ensure these modules are present in your directory
from config import (
...
from reconcile_page import ReconcilePanel
...
# Correct pathing for bundled macOS apps
_LICENSE_PATH = resource_path("LICENSE.txt")

def _open_license():
    if not os.path.exists(_LICENSE_PATH):
        messagebox.showinfo("Licence",
                            "LICENSE.txt not found in the application bundle.")
        return
    try:
...
class TransformerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Mac Retina Display friendly sizing
    ...
    def _set_app_icon(self):
        """macOS uses .icns for the bundle, but tkinter needs specific handling."""
        try:
            ...
            # Fallback if PIL is not installed or file is missing
            pass

    def _apply_styles(self):
        style = ttk.Style(self)
        # 'clam' is the most consistent cross-platform theme for custom colors
    ...
    def _build_ui(self):
        ...
        # Using Helvetica/System font for Mac aesthetics
        main_font = "Helvetica" if sys.platform == "darwin" else "Segoe UI"
        ...
        # Action Buttons
        for icon, label, cmd in [("⚖", "Reconcile", self._toggle_reconcile),
                                 ("📝", "Notes", self._toggle_notes),
                                 ("⚙", "Settings", self._toggle_settings)]:
            btn_f = tk.Frame(icons_f, bg=THEME_BG)
            btn_f.pack(side="left", padx=10)
            tk.Button(btn_f, text=icon,
                      font=("Apple Color Emoji" if sys.platform == "darwin" else "Segoe UI Symbol", 16),
                      bg=THEME_BG, fg=TEXT_COLOR,
                      activebackground=THEME_ACCENT,
                      relief="flat", bd=0, cursor="hand2",
                      command=cmd).pack()
            tk.Label(btn_f, text=label, font=FONT_SMALL, bg=THEME_BG, fg=TEXT_COLOR).pack()
        ...
        ttk.Separator(self, orient="horizontal").pack(fill="x", padx=15, pady=(10, 5))
        ...
        self.reconcile_panel = ReconcilePanel(self)
    ...
    def _build_controls_left(self):
        ...
        tk.Label(mode_f, text="Mode:", font=FONT_BOLD, bg=THEME_BG, fg=TEXT_COLOR).pack(side="left", padx=(0, 5))
        self.mode_box = ttk.Combobox(mode_f, textvariable=self.mode_var, values=MODE_OPTIONS, state="readonly", width=14, font=FONT_BODY)
        ...
        tk.Label(date_f, text="Original CSV Date Order:", font=FONT_BOLD, bg=THEME_BG, fg=TEXT_COLOR).pack(side="left", padx=(0, 5))
        self.date_box = ttk.Combobox(date_f, textvariable=self.date_sequence_var, values=DATE_SEQUENCE_OPTIONS, state="readonly", width=12, font=FONT_BODY)
        ...
        self.btn_load = ttk.Button(btn_row, text="1. 📂 Load File", command=self.load_file, state="disabled")
        self.btn_load.pack(side="left", padx=3)
        tk.Label(btn_row, text=" → ", font=("Helvetica", 12), bg=THEME_BG, fg=THEME_ACCENT).pack(side="left")
        
        self.btn_prev_orig = ttk.Button(btn_row, text="2. 📥 Original", command=self.preview_original, state="disabled")
        self.btn_prev_orig.pack(side="left", padx=3)
        tk.Label(btn_row, text=" → ", font=("Helvetica", 12), bg=THEME_BG, fg=THEME_ACCENT).pack(side="left")
        
        self.btn_prev_trans = ttk.Button(btn_row, text="3. ✨ Transformed", command=self.preview_transformed, state="disabled")
        self.btn_prev_trans.pack(side="left", padx=3)
        tk.Label(btn_row, text=" → ", font=("Helvetica", 12), bg=THEME_BG, fg=THEME_ACCENT).pack(side="left")
        
        self.btn_exec = ttk.Button(btn_row, text="4. ⚙️ Process", command=self.transform_action, state="disabled")
    ...
    def _build_footer(self):
        ...
        tk.Label(footer, text="© 2026 4G Capital | Internal Systems",
                 font=("Helvetica", 8), bg="#c9a66b", fg="#3a2f24").pack(side="left", padx=15, pady=5)
        tk.Label(footer, text="v1.0.3", font=("Helvetica", 8),
                 bg="#c9a66b", fg="#5a4530").pack(side="right", padx=15, pady=5)
        lic = tk.Label(footer, text="📄 View Software Licence", font=("Helvetica", 8, "underline"),
    ...
    def _toggle_reconcile(self):
        ...
        if self.reconcile_panel.winfo_ismapped(): self._hide_overlay(self.reconcile_panel)
        else: self._show_overlay(self.reconcile_panel)

    def _toggle_notes(self):
        for p in [self.settings_panel, self.reconcile_panel]:
            if p.winfo_ismapped(): self._hide_overlay(p)
        if self.notes_panel.winfo_ismapped(): self._hide_overlay(self.notes_panel)
        else: self._show_overlay(self.notes_panel); self.notes_panel.show()

    def _toggle_settings(self):
        for p in [self.notes_panel, self.reconcile_panel]:
            if p.winfo_ismapped(): self._hide_overlay(p)
        if self.settings_panel.winfo_ismapped(): self._hide_overlay(self.settings_panel)
        else: self._show_overlay(self.settings_panel); self.settings_panel.show()
    ...
    def _mode_changed(self, event=None):
        ...
        render_instructions(self.user_action_frame, self.system_logic_frame, mode, self.ref_guide_frame if mode == "JSON Generator" else None)
    ...
    def load_file(self):
        path = filedialog.askopenfilename(title="Select CSV", filetypes=[("CSV Files", "*.csv;*.txt"), ("All Files", "*.*")])
        if not path: return
    ...
    def transform_action(self):
        ...
        try:
            ...
            # macOS friendly success message
            messagebox.showinfo("Success", f"Saved {len(saved)} file(s) to:\n{out_dir}")
            # Automatically open the folder on Mac
            if sys.platform == "darwin": subprocess.call(["open", out_dir])
